...
def validate_course_response(course_title: str) -> bool:
    """
    Validate that the course title is in the database.
    """
    collection = get_vector_db_collection()
    return course_title in collection.get()["ids"]
...
if __name__ == "__main__":
    ...
    status = args.status
    query = args.query
    # Grab a set of the course ids for validation later
    collection = get_vector_db_collection()
    course_set = set(collection.get()["ids"])
    if args.readme:
        print_readme()
        sys.exit()
    if status:
    ...
    if args.input_file:
        ...
        # Validate results
        for result in results:
            if result[0] not in course_set:
                console.print(
                    f"[red]HALLUCINATION? Course ID not found in database: {result[0]}[/red]"
                )
